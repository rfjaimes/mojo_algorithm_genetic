...
def load_distances() -> DynamicVector[Int]:
...
def load_cities(city_count) -> Matrix[DType.float32]:
    # let np = Python.import_module("numpy")
    cities = Matrix[DType.float32](int(city_count), 2)
    cities.selected = load_distances()
    return cities


def generate_permutation(city_count: Int) -> DTypePointer[DType.int32]:
...
def calculate_distance(pos_city1: Int, post_city2: Int, inout cities: Matrix[DType.float32]) -> Float32:
...
def calculate_distance_lineal(pos_city1: Int, post_city2: Int, cities: Matrix[DType.float32]) -> Float32:
    # Función para calcular la distancia entre dos ciudades (puede ser la distancia euclidiana)
    # let position = post_city2 * 251 + pos_city1
...
def total_distance(inout population: Matrix[DType.int32], y: Int, inout cities: Matrix[DType.float32], city_count: Int) -> SIMD[DType.float32, 1]:
...
def evaluate_population(inout population: Matrix[DType.int32], inout cities: Matrix[DType.float32], city_count: Int, population_size: Int) -> DynamicVector[SIMD[DType.float32, 1]]:
...
def containInArray(array: DynamicVector[Int], value: Int) -> Bool:
    for i in range(array.size):
        if array[i] == value:
            return True
    return False

def printVector(vector: DynamicVector[Int]):
...
def crossover(parentId1: Int, parentId2: Int, inout population: Matrix[DType.int32]) -> Int:
    # Implementa el operador de cruce (crossover) para generar dos descendientes a partir de dos padres
    crossover_point = (random_float64() * (population.cols - 1)).to_int() 
    ...
    for i in range(population.cols):
        if not containInArray(parent1_left, population.__getitem__(parentId2, i).to_int()):
            parent1_right.append(population.__getitem__(parentId2, i).to_int())

        if not containInArray(parent2_left, population.__getitem__(parentId1, i).to_int()):
    ...
    new_children1_id = population.not_selected.pop_back()
...
def mutate(inout population: Matrix[DType.int32], populationId: Int):
    # Implementa el operador de mutación para perturbar la ruta de manera aleatoria
    mutation_points = generate_permutation(population.cols)
    # print("Mutation points: ", mutation_points[0], " ", mutation_points[1])
    temp = population.__getitem__(populationId, mutation_points[0].to_int())
    population.__setitem__(populationId, mutation_points[0].to_int(), population.__getitem__(populationId, mutation_points[1].to_int()))
    population.__setitem__(populationId, mutation_points[1].to_int(), temp)


def getIndexSorted(values: DynamicVector[SIMD[DType.float32, 1]]) -> DynamicVector[Int]:
...
def evolve_population(inout population: Matrix[DType.int32]):
    ...
    for i in range(population.selected.size):
        tournament_indices = generate_permutation(population.selected.size)
        parent1 = tournament_indices[0].to_int()
        parent2 = tournament_indices[1].to_int()

        # Aplicar cruce con una cierta probabilidad
        if random_float64() < crossover_probability:
            new_id = crossover(population.selected[parent1], population.selected[parent2], population)
...
def select_elite(inout population: Matrix[DType.int32], fitness_values: DynamicVector[SIMD[DType.float32, 1]], population_size: Int):
...
def main():
    ...
    for i in range(generations):
        var fitness_values:DynamicVector[SIMD[DType.float32, 1]] = evaluate_population(population, cities, city_count, population_size)

        select_elite(population, fitness_values, population_size)
        print("Generacion: ", i,"Best route: ", population.selected[0], " Distance: ", fitness_values[population.selected[0]])
